...
exports.store = async (req, res) => {
  try {
    ...
    return res.status(200).json({ status: true, message: "finally, Category has been Created by the admin.", category: category });
  } catch (error) {
    if (req.file) deleteFile(req.file);
    console.log(error);
...
exports.update = async (req, res) => {
  try {
    ...
    return res.status(200).json({ status: true, message: "finally, category has been Updated by the admin.", category: category });
  } catch (error) {
    if (req.file) deleteFile(req.file);
    console.log(error);
    return res.status(500).json({
...
exports.destroy = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ status: true, message: "finally, ctaegory and subcategories associated with the category deleted by admin." });
  } catch (error) {
    console.log(error);
    return res.status(500).json({
      status: false,
...
exports.get = async (req, res) => {
  try {
    const [category] = await Promise.all([Category.find().populate("subCategory", "name image").sort({ createdAt: -1 })]);
    ...
    return res.status(200).json({
      status: true,
      message: "Finally, get all categories by the seller.",
      category,
    });
  } catch (error) {
    console.log(error);
