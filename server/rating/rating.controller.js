  ...
  try {
    ...
    if (ratingExist) {
      return res.status(200).json({
        status: false,
        message:
          "You don't have right to give rate because you have already rated on this product!!",
      });
    }
    ...
    return res.status(200).json({
      status: true,
      message: "finally, rating given by this user for this product!!",
      rating,
    });
  } catch (error) {
    console.log(error);
...
exports.getRating = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ status: true, message: "Success!!", totalRating });
  } catch (error) {
    console.log(error);
    return res.status(500).json({
      status: false,
