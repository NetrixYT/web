...
exports.update = async (req, res) => {
  try {
    if (!req.query.promoCodeId) return res.status(200).json({ status: false, message: "promoCodeId must be requried!!" });
...
exports.destroy = async (req, res) => {
  try {
    ...
    const promoCode = await PromoCode.findById(req.query.promoCodeId);
    if (!promoCode) return res.status(200).json({ status: false, message: "promoCode does not found!!" });
...
exports.index = async (req, res) => {
  try {
    const promoCode = await PromoCode.find().sort({
      createdAt: 1,
    });

    if (!promoCode) return res.status(200).json({ status: false, message: "No data found." });
