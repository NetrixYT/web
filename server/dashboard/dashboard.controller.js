...
exports.dashboard = async (req, res) => {
  try {
    const [totalProducts, totalOrders, totalUsers, totalSeller, totalLiveSeller] = await Promise.all([
      Product.find().countDocuments(),
      Order.find().countDocuments(),
      User.find().countDocuments(),
      Seller.find().countDocuments(),
...
exports.analyticOfUsers = async (req, res) => {
  try {
    ...
    const [todayUsers, yesterdayUsers, weeklyUsers, analyticdata] = await Promise.all([
      ...
      User.aggregate([
        { $addFields: { analyticDate: { $toDate: "$createdAt" } } },
