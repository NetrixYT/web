...
exports.acceptUpdateRequest = async (req, res) => {
  try {
    if (!req.query.requestId) return res.status(200).json({ status: false, message: "requestId must be requried." });

    const updateRequest = await ProductRequest.findOne({ _id: req.query.requestId, updateStatus: "Pending" });
    if (!updateRequest) return res.status(200).json({ status: false, message: "product request to update the product does not found." });
...
exports.updateProductRequestStatusWise = async (req, res) => {
  try {
    if (!req.query.status) return res.status(200).json({ status: true, message: "Oops ! Invalid details." });
