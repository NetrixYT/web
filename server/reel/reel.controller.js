...
exports.uploadReel = async (req, res) => {
  try {
    ...
    if (!product) { return res.status(200).json({ status: false, message: "Product does not found!" }) };
...
exports.getReelsForUser = async (req, res) => {
  try {
    ...
    const [user, setting] = await Promise.all([User.findOne({ _id: req.query.userId }),
