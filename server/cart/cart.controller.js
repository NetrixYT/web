...
exports.addToCart = async (req, res) => {
  try {
    ...
    //console.log("attributesArray: ", req.body.attributesArray);
    //console.log("attributesArray: ", typeof req.body.attributesArray);
    ...
    if (cart) {
      ...
      return res.status(200).json({
        status: true,
        message: "finally, product added to cart.",
        data: data,
      });
    } else {
  ...
  try {
    ...
    if (cartByUser?.items?.length) {
      if (Array.isArray(attributes)) {
        const updateRecordIndex = cartByUser.items.findIndex(
          (item) =>
            item?.productId.toString() === product._id.toString() && JSON.stringify(item?.attributesArray) === JSON.stringify(attributes)
        );
    ...
    return res.status(200).json({
      status: true,
      message: "finally, product removed from cart!!",
      data: data,
    });
  } catch (error) {
    console.log(error);
...
exports.getCartProduct = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      status: true,
      message: "Finally, get all products added to cart.",
      data: populatedCart,
    });
  } catch (error) {
    console.log(error);
...
exports.deleteCart = async (req, res) => {
  try {
    ...
    return res.status(200).json({ status: true, message: "finally, remove all products from the cart for this user!" });
  } catch (error) {
    console.log(error);
