...
exports.store = async (req, res) => {
  try {
    ...
    if (!userEmail) { return res.status(200).json({ status: false, message: "User does not found with that email." }) }
    ...
    var mailOptions = {
      from: `${config.EMAIL}`,
      to: req.body.email,
      subject: "Sending Email from EraShop for Password Security",
      html: tab,
    };
