...
exports.getreview = async (req, res) => {
  try {
    if (!req.query.productId) return res.status(200).json({ status: false, message: "OOps ! Invalid details!!" });
...
exports.destroy = async (req, res) => {
  try {
    ...
    if (!review) {
      return res.status(200).json({ status: false, message: "reviewId does not found." });
    }
