...
exports.detailforSeller = async (req, res) => {
  try {
    if (!req.query.productId || !req.query.sellerId) { return res.status(200).json({ status: false, message: "Oops ! Invalid details." }) }
    ...
    if (!seller) { return res.status(200).json({ status: false, message: "seller does not found!!" }) }
...
exports.deleteProduct = async (req, res) => {
  try {
    if (!req.query.productId) return res.status(200).json({ status: false, message: "productId must be requried!" });

    const [product] = await Promise.all([
      Product.findOne({ _id: req.query.productId, createStatus: "Approved" }),
    ]);
...
exports.getSelectedProducts = async (req, res) => {
  try {
    ...
    const [seller, totalSelectedProducts, selectedProducts] = await Promise.all([
      Seller.findById(req.query.sellerId),
      Product.countDocuments({ isSelect: true, seller: req.query.sellerId }),
      Product.find({ isSelect: true, seller: req.query.sellerId })
    ...
    const liveSeller = await Seller.aggregate([
      {
        $match: {
          isBlock: false,
          isLive: true,
...
exports.productDetail = async (req, res) => {
  try {
    if (!req.query.productId || !req.query.userId) return res.status(200).json({ status: true, message: "Oops ! Invalid details!!" });
...
exports.filterWiseProduct = async (req, res) => {
  try {
    if (!req.query.userId) return res.status(200).json({ status: true, message: "userId must be requried!" });
...
exports.getSellerWise = async (req, res) => {
  try {
    if (!req.query.sellerId) return res.status(200).json({ status: false, message: "Oops! Invalid details!!" });
