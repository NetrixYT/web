...
exports.liveSeller = async (req, res) => {
  try {
    ...
    const [seller, SelectedProducts] = await Promise.all([
      Seller.findById(req.body.sellerId),
      Product.find({ seller: req.body.sellerId, isSelect: true }).select("mainImage productName price seller isSelect attributes"),
    ]);
    ...
    const existLiveSeller = await LiveSeller.findOne({ sellerId: seller._id });

    const liveSeller = new LiveSeller();

    let liveSellerData;
    if (existLiveSeller) {
      existLiveSeller.liveSellingHistoryId = liveSellingHistory._id;
      existLiveSeller.agoraUID = req?.body?.agoraUID;
      existLiveSeller.selectedProducts = SelectedProducts;

      liveSellerData = await liveSellerFunction(existLiveSeller, seller);
    } else {
    ...
    const follower = await Follower.find({ sellerId: seller._id }).distinct("userId");
    ...
    let matchQuery = {};
    if (existLiveSeller) {
      matchQuery = { $match: { _id: { $eq: existLiveSeller._id } } };
    } else {
      matchQuery = { $match: { _id: { $eq: liveSeller._id } } };
    }

    const data = await LiveSeller.aggregate([matchQuery]);
