...
io.on("connect", async (socket) => {
  ...
  socket.on("endLiveSeller", async (data) => {
    ...
    if (seller) {
      ...
      const socketsInRoom = io.sockets.adapter.rooms.get("liveSellerRoom:" + parsedData.liveSellingHistoryId);
      console.log("socketsInRoom before:         ", socketsInRoom);

      if (socketsInRoom) {
        for (const socketId of socketsInRoom) {
          io?.sockets?.sockets?.get(socketId).leave("liveSellerRoom:" + parsedData.liveSellingHistoryId);
        }
      }

      const abcd = io.sockets.adapter.rooms.get("liveSellerRoom:" + parsedData.liveSellingHistoryId);
      console.log("socketsInRoom after:         ", abcd);
    }
  });
  ...
  socket.on("disconnect", async (data) => {
    ...
    if (seller) {
      ...
      const socketsInRoom = io.sockets.adapter.rooms.get("liveSellerRoom:" + seller.liveSellingHistoryId);
      console.log("socketsInRoom in disconnect:   ", socketsInRoom);

      if (socketsInRoom) {
        for (const socketId of socketsInRoom) {
          io?.sockets?.sockets?.get(socketId).leave("liveSellerRoom:" + seller.liveSellingHistoryId);
        }
      }
    }
  });
